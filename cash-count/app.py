...
def hello_world():
    ...
    history = db.execute(
        "SELECT user_id, register, date, time, variance FROM history WHERE DATE(date) = DATE('now')"
    )
...
def history():
    ...
    if request.method == "POST":
        ...
        if time == 'week':
            #date format help from the duck
            history = db.execute(
                "SELECT user_id, register, date, time, variance FROM history WHERE strftime('%W', date) = strftime('%W', 'now')"
            )
        ...
        if time == 'month':
            #date format help from the duck
            history = db.execute(
                "SELECT user_id, register, date, time, variance FROM history WHERE strftime('%m', date) = strftime('%m', 'now')"
            )
        ...
        if time == 'year':
            #date format help from the duck
            history = db.execute(
                "SELECT user_id, register, date, time, variance FROM history WHERE strftime('%Y', date) = strftime('%Y', 'now')"
            )
...
def variances():
    ...
    if request.method == "POST":
        ...
        if time == 'week':
            #date format help from the duck
            history = db.execute(
                "SELECT user_id, register, date, time, variance FROM history WHERE variance != 0 AND strftime('%W', date) = strftime('%W', 'now')"
            )
        ...
        if time == 'month':
            #date format help from the duck
            history = db.execute(
                "SELECT user_id, register, date, time, variance FROM history WHERE variance != 0 AND strftime('%m', date) = strftime('%m', 'now')"
            )
        ...
        if time == 'year':
            #date format help from the duck
            history = db.execute(
                "SELECT user_id, register, date, time, variance FROM history WHERE variance != 0 AND strftime('%Y', date) = strftime('%Y', 'now')"
            )
